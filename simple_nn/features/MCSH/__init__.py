...
class MCSH_function(object):
    def __init__(self, inputs=None):
        self.parent = None
        self.key = 'MCSH_function'
        self.default_inputs = {'MCSH_function': 
                                  {
                                      'params': dict(),
    ...
    def set_inputs(self):
        self.inputs = self.parent.inputs['MCSH_function']
    ...
    def preprocess(self, calc_scale=True, use_force=False, use_stress=False, get_atomic_weights=None, **kwargs):
        ...
        params_set['i'], params_set['d'] = _read_mcsh_params(self.inputs['mcsh_params'])
        if calc_scale:
    ...
    def generate(self):
        ...
        for j,jtem in enumerate(self.parent.inputs['atom_types']):
            ...
            params_set[jtem]['gaussian_params'], params_set[jtem]['gaussian_count'] = _read_atom_gaussian_params(self.inputs['params']['gaussian_params'][jtem])
        ...
        for item, ind in zip(structures, structure_ind):
            ...
            for atoms in snapshots:
                ...
                # atom_i is the type index, starting at 1
                atom_i_p = ffi.cast("int *", atom_i.ctypes.data)

                # type_num example: {'O': 22, 'H': 44}
                # type_idx example: {'O': array([ 0,  1,  4,  9, 12, 16, 19, 21, 24, 27, 30, 33, 37, 39, 42, 45, 49,
                    # 51, 55, 58, 61, 63]), 'H': array([ 2,  3,  5,  6,  7,  8, 10, 11, 13, 14, 15, 17, 18, 20, 22, 23, 25,
                    # 26, 28, 29, 31, 32, 34, 35, 36, 38, 40, 41, 43, 44, 46, 47, 48, 50,
                    # 52, 53, 54, 56, 57, 59, 60, 62, 64, 65])}
                ...
                for j,jtem in enumerate(self.parent.inputs['atom_types']):
                    ...
                    # indices of atom in the snapshot (atoms) that are going to be calulated
                    ...
                    # extern "C" int calculate_calculate_atomistic_mcsh(double** cell, double** cart, double** scale,
                                        # int* atom_i, int natoms, int* cal_atoms, int cal_num,
                                        # int** params_i, double** params_d, int nmcsh, double** atom_gaussian, int* ngaussian,
                                        # double** mcsh, double** dmcsh, double** dmcsha)
